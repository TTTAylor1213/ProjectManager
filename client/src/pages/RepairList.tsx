...
import { repairStatusMap } from "./statusLabels";
import dayjs from "dayjs";

interface Repair {
  id: number; fault_description: string; repair_status: string; sent_date: string; returned_date: string | null;
  repair_provider: string; cost: number; handlerId: number | null; deviceId: number | null; projectId: number | null;
  device_name?: string; project_name?: string; handler_name?: string;
}
...
export default function RepairList() {
  ...
  const columns: ColumnsType<Repair> = [
    ...
    { title: "状态", dataIndex: "repair_status", width: 100, render: (s: string) => { const c = repairStatusMap[s]; return c ? <Tag color={c.color}>{c.label}</Tag> : s; } },
    { title: "送修日期", dataIndex: "sent_date", width: 110, render: (d: string) => d ? dayjs(d).format("YYYY-MM-DD") : "-" },
    { title: "返回日期", dataIndex: "returned_date", width: 110, render: (d: string) => d ? dayjs(d).format("YYYY-MM-DD") : "-" },
    { title: "维修方", dataIndex: "repair_provider", width: 120 },
    ...
    <div>
      ...
      <Space style={{ marginBottom: 16 }} wrap>
        ...
        <Select placeholder="筛选状态" value={statusFilter} onChange={setStatusFilter} allowClear style={{ width: 140 }}
          options={Object.entries(repairStatusMap).map(([k, v]) => ({ value: k, label: v.label }))} />
        <Button type="primary" icon={<PlusOutlined />} onClick={handleAdd}>添加维修</Button>
        <Button icon={<ExportOutlined />} onClick={async () => { const blob = await exportExcel("/export/repair"); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = `repairs.xlsx`; a.click(); URL.revokeObjectURL(url); }}>导出Excel</Button>
      </Space>
      ...
      <Modal title={editing ? "编辑维修" : "添加维修"} open={modalOpen} onOk={handleSubmit} onCancel={() => setModalOpen(false)} width={600} destroyOnClose>
        <Form form={form} layout="vertical">
          ...
          <Form.Item name="repairStatus" label="状态" initialValue="pending">
            <Select options={Object.entries(repairStatusMap).map(([k, v]) => ({ value: k, label: v.label }))} />
          </Form.Item>
